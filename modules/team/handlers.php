	...
	$rows = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT tm.id, tm.member_user_id, tm.role, tm.invited_at, tm.accepted_at,
			 ...
			 FROM {$wpdb->prefix}clientflow_team_members tm
...
function clientflow_team_invite_member( int $owner_id, string $email, string $name, string $role ): array {
	...
	$seats_used  = (int) $wpdb->get_var( $wpdb->prepare(
		"SELECT team_seats_used FROM {$wpdb->prefix}clientflow_user_meta WHERE user_id = %d FOR UPDATE",
		$owner_id
	) );
	...
	$existing_row = $wpdb->get_var( $wpdb->prepare(
		"SELECT id FROM {$wpdb->prefix}clientflow_team_members WHERE owner_id = %d AND member_user_id = %d",
		$owner_id,
		$member_user_id
	) );
	...
	$wpdb->insert(
		$wpdb->prefix . 'clientflow_team_members',
		[
			'owner_id'       => $owner_id,
	...
	$wpdb->query( $wpdb->prepare(
		"UPDATE {$wpdb->prefix}clientflow_user_meta SET team_seats_used = team_seats_used + 1 WHERE user_id = %d",
		$owner_id
	) );
...
function clientflow_team_send_invite_email( int $member_user_id, int $owner_id, bool $is_new_user ): void {
	...
	$business_name = get_option( 'clientflow_business_name', get_bloginfo( 'name' ) );
	$login_url     = admin_url();
