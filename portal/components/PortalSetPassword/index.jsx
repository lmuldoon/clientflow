...
export default function PortalSetPassword() {
	...
	const handleSubmit = useCallback( async ( e ) => {
		...
		try {
			const res = await fetch( window.cfPortalData.apiUrl + '/portal/set-password', {
				method:  'POST',
				headers: {
			...
			if ( res.success ) {
				setRedirectUrl( res.redirect_url || '/clientflow/dashboard' );
			} else {
	...
	const loginUrl = ( window.location.origin || '' ) + '/clientflow/login';
