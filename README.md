...
ClientFlow gives freelancers and agencies everything they need to send proposals, collect payments, manage projects, and communicate with clients — all from the WordPress admin.
...
1. Upload the `clientflow` folder to `wp-content/plugins/` and activate the plugin.
2. The setup wizard launches automatically on first activation.
3. Complete the wizard: connect Stripe, add your brand details.
4. Activate your Pro or Agency licence via **ClientFlow → Account** (powered by Freemius). Your plan upgrades automatically — no key entry required.

---

## Licensing

ClientFlow uses [Freemius](https://freemius.com) for licence management.

- Purchase a Pro or Agency licence at [wpclientflow.co.uk](https://wpclientflow.co.uk).
- Activate the licence via **ClientFlow → Account** in the WordPress admin.
...
Set a deposit percentage to collect upfront, with the balance charged when the project completes. Or require full payment at acceptance. ClientFlow creates Stripe Checkout sessions automatically — you never handle card details.

---

## Client Portal (Pro/Agency)

Accepted clients get access to a branded portal at `/clientflow/` on your site. They log in via magic link (emailed automatically on acceptance) or a password they set themselves.
...
AI requests are proxied through the ClientFlow relay server — your OpenAI costs are included in the licence. Monthly limits: 100 requests (Pro), 500 requests (Agency). The counter resets on the 1st of each month.

---

## Webhooks (Pro/Agency)

Connect ClientFlow to Zapier, Make, or any other automation tool using outgoing webhooks.
...
```
X-ClientFlow-Event: proposal.accepted
X-ClientFlow-Signature: sha256=<hmac>
```
...
2. Copy the Zapier endpoint URL into ClientFlow → Webhooks.
3. Subscribe to the events you want to act on.
4. Test the webhook from the ClientFlow admin to confirm delivery.

---

## Analytics (Pro/Agency)

**ClientFlow → Analytics** shows revenue KPIs, a revenue-over-time chart, proposal performance breakdown (sent / viewed / accepted / declined), and a live activity feed. Export to CSV for any date range up to one year.

---

## Testimonial Emails

ClientFlow can automatically send a review request email when a project is completed and fully paid. Configure the message, review link (Google, Trustpilot, etc.), and button label under **Settings**.

---

## Team Members (Agency)

Invite team members under **ClientFlow → Team**. Assign one of three roles:
...
All endpoints are under `/wp-json/clientflow/v1/` and require a logged-in WordPress user (`X-WP-Nonce` header) unless noted.
...
Never add `if ($plan === 'pro')` checks in module code — always call `cf_can_user()` or `ClientFlow_Entitlements::can_user()`.
...
| Table | Purpose |
|---|---|
| `clientflow_user_meta` | Plan, usage counters, billing metadata |
| `clientflow_clients` | Client records |
| `clientflow_proposals` | Proposal content and status |
| `clientflow_payments` | Stripe payment records |
| `clientflow_projects` | Post-acceptance project tracking |
| `clientflow_milestones` | Per-project milestones |
| `clientflow_messages` | Client ↔ agency message threads |
| `clientflow_files` | Project file uploads |
| `clientflow_approvals` | Client approval requests |
| `clientflow_webhooks` | Registered webhook endpoints |
| `clientflow_webhook_logs` | Delivery history (last 3 per webhook) |
| `clientflow_events` | Analytics event log |
| `clientflow_ai_usage_logs` | AI request audit trail |
| `clientflow_team_members` | Team member roles |
